...
Any both ESL enrollment per semester (1 for credit, 0 for not enrolled IN BOTH) #}
    per_term_info as (
        select
            ...
            case
                when min(c.cb03) = '493087' then true else false
            end as only_integrated_esl_course
    ...
    student_codes as (
        select
            s.uuid,
            string_agg(
                p.any_esl_enrollment::int::varchar, '' order by p.term
            ) as any_esl_enrollment_by_term,
            string_agg(
                p.any_noncredit_esl_enrollment::int::varchar, '' order by p.term
            ) as any_noncredit_esl_enrollment_by_term,
            string_agg(
                p.any_credit_esl_enrollment::int::varchar, '' order by p.term
            ) as any_credit_esl_enrollment_by_term,
            string_agg(
                p.any_both_esl_enrollment::int::varchar, '' order by p.term
            ) as any_both_esl_enrollment_by_term,
            string_agg(
                p.first_cb21_level_adjusted_for_college, '' order by p.term
            ) as first_cb21_level_adjusted_for_college_by_term,
            string_agg(
                p.only_integrated_esl_course::int::varchar, '' order by p.term
            ) as bool_only_integrated_esl_course_by_term
        from students s
...
select
    s.*,
    sc.any_esl_enrollment_by_term,
    sc.any_noncredit_esl_enrollment_by_term,
    sc.any_credit_esl_enrollment_by_term,
    sc.any_both_esl_enrollment_by_term,
    sc.first_cb21_level_adjusted_for_college_by_term,
    sc.bool_only_integrated_esl_course_by_term
